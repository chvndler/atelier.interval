var comp;
//DIRECTORY C:\Program Files\Adobe\Adobe After Effects 2020\Support Files\Plug-ins
...
function shakeChange(value) {
  //OK //default 22
  for (i = 1; i < comp.numLayers; i++) {
    if (comp.layer(i).name == 'TRANSFORM') {
      var effect = comp.layer(i);
      var prop = '([position[0],wiggle(44,' + value + ')[1]])';
      effect.property('ADBE Effect Parade').property(1).property('Position').expression = prop;
    }
  }
}
...
function toggleDirtyEdges(isToggle) {
  for (i = 1; i < comp.numLayers; i++) {
    if (comp.layer(i).name == '09-DIRTY_EDGES.mov') {
      if (isToggle == 'true') {
        comp.layer(i).enabled = true;
      }
      if (isToggle == 'false') {
...
function deleteFiles() {
  ...
  for (k = 0; k < list.length; k++) {
    for (l = 1; l < app.project.numItems + 1; l++) {
      ...
      } else if (app.project.item(l).name == 'RollBack8-Assets') {
        app.project.item(l).remove();
      }
    }
  }
...
function importToComp() {
  ...
  try {
    ...
    // Working with comp "ROLLBACK8", varName "comp";
...
function precompileList(layerName) {
  ...
  comp.layers.precompose(newList, 'Rollback8', true);
  /*
...
function createFolder() {
  var folder_properties = { name: 'RollBack8 Assets', typeName: 'Folder', label: 2, comment: '' };
  assetsFolder = findProjectItem(app.project.rootFolder, false, folder_properties);
  if (assetsFolder === null) {
    assetsFolder = app.project.items.addFolder('RollBack8-Assets');
  }
}
