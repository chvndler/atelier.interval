...
function createSelectedStyle(value) {
  //OK //default 80
  for (i = 1; i < comp.numLayers; i++) {
    if (comp.layer(i).name == 'Atelier®Default') {
      var effect = comp.layer(i);
      effect.property('ADBE Effect Parade').property(1).property('ADBE Lumetri-0065').setValue(100);
    }
  }
}
...
function toggleGrain(isToggle) {
  for (i = 1; i < comp.numLayers; i++) {
    if (comp.layer(i).name == '03-GRAIN.mxf') {
      if (isToggle == 'true') {
...
function importToComp() {
  ...
  try {
    ...
    var timecodeAV = findProjectItem(assetsFolder, false, timecodeAV_properties);
    if (timecodeAV === null) {
      var tempSolid = comp.layers.addSolid(col, timecodeAV_properties.name, compWidth, compHeight, px);
      timecodeAV = tempSolid.source;
      timecodeAV.parentFolder = assetsFolder;
      tempSolid.remove();
    }
    ...
    var interval = comp.layers.add(timecodeAV);
    interval.name = 'INTERVAL';

    interval.outPoint = layerLength;
    interval.textLayer = true;
    interval.selected = false;
    // Apply expressions to properties
    try {
      interval.property('ADBE Effect Parade').addProperty('ADBE Current Time Format');
      text.sourceText.createStyle().setFontSize(300).setFont('Impact').expression = `function pad(str, max) {
        	str = str.toString();
    ...
    try {
      ...
      transform.property('ADBE Effect Parade').property(1).property('Position').expression =
        '([position[0],wiggle(4,22)[1]])';
    } catch (err) {}

    // Add existing Solid Layer "ADJUSTMENT LAYER", varName "flickerAV";
    ...
    // Add existing Solid Layer "ADJUSTMENT LAYER ( colorGlowAV )", varName "colorGlowAV";
    ...
    colorGlow
      ...
      .property(1)
      .property('ADBE Lumetri-0012') // Contrast -50
      .setValue(-50);
    colorGlow
      .property('ADBE Effect Parade')
      .property(1)
      .property('ADBE Lumetri-0013') // Highlights -36
      .setValue(-36);
    colorGlow
      .property('ADBE Effect Parade')
      .property(1)
      .property('ADBE Lumetri-0015') // Whites 22
      .setValue(22);
    colorGlow
      .property('ADBE Effect Parade')
      .property(1)
      .property('ADBE Lumetri-0065') // Saturation 100
      .setValue(100);
    ...
    colorGlow
      .property('ADBE Effect Parade')
      .property(2)
      .property('ADBE Glo2-0003') // Glow Radius 300
      .setValue(300);
    colorGlow
      .property('ADBE Effect Parade')
      .property(2)
      .property('ADBE Glo2-0004') // Glow Intensity 0.1
      .setValue(0.10000000149012);
    colorGlow.selected = false;

    // Add existing Solid Layer "ADJUSTMENT LAYER", varName "redBlurAV";
    ...
    redBlur
      .property('ADBE Effect Parade')
      .property(1)
      .property('ADBE Channel Blur-0001') // RED BLUR (100)
      .setValue(100);
    redBlur
      .property('ADBE Effect Parade')
      .property(1)
      .property('ADBE Channel Blur-0002') // GREEN BLUR (22)
      .setValue(22);
    redBlur
      .property('ADBE Effect Parade')
      .property(1)
      .property('ADBE Channel Blur-0005') // EDGE BEHAVIOR (ON)
      .setValue(1);
    redBlur
      .property('ADBE Effect Parade')
      .property(1)
      .property('ADBE Channel Blur-0006') // BLUR DEMENSIONS (Vertical(3))
      .setValue(3);
    ...
    redBlur
      .property('ADBE Effect Parade')
      .property(2)
      .property('ADBE Gaussian Blur 2-0001') // GAUSSIAN BLURRINESS (44)
      .setValue(44);
    redBlur
      .property('ADBE Effect Parade')
      .property(2)
      .property('ADBE Gaussian Blur 2-0002') // BLUR DEMENSIONS (horizontal(2))
      .setValue(2);
    redBlur
      .property('ADBE Effect Parade')
      .property(2)
      .property('ADBE Gaussian Blur 2-0003') // REPEAT EDGE PIXELS (off default)
      .setValue(1);
